...
if __name__ == '__main__':
    ...
    classifier = GinClassifier(emb_size=out_channels, num_classes=labels.shape[1])
    
    # cpu = torch.device('cpu') 


    start_time = time.time()
    loss_over_time = train(gin, classifier, labels, adj_list, train_nodes)
    end_time = time.time()
    ...
    start_time = time.time()
    precision_score, recall_score, f1_score = test(gin, classifier, test_nodes, labels, adj_list)
    end_time = time.time()
    ...
    precision_score, recall_score, f1_score = test_rare_diseases(gin, classifier, test_nodes, labels, adj_list, rare_diseases)
